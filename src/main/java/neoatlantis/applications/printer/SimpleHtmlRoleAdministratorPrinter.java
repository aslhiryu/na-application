...
public class SimpleHtmlRoleAdministratorPrinter extends HtmlRoleAdministratorPrinter {
    ...
    public String contentEditPermissions(Map<String,Object> params) throws FormatterException{
        ...
        for(int i=0; disponibles!=null&&i<disponibles.size(); i++){
            ...
            sb.append("\" title=\"").append(selecionado? "Remueve": "Asigna").append(" Permiso\" onclick=\"NAAdminUsers")
                    .append(selecionado? "Remove": "Add").append("PermissionRole('")
                    .append(((Role)params.get(UtilsAuthenticatorBean.ROLE_PARAM)).getId()).append("', '").append(disponibles.get(i).getId())
                    .append("', '").append(uCon.getSession().getId()).append("')\">&nbsp;</a>")
                    .append(disponibles.get(i).getName()).append("<br />\n");
        }
        sb.append("</td>\n");
        sb.append("</tr>\n");
