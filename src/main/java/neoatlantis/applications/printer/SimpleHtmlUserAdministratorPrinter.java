...
public class SimpleHtmlUserAdministratorPrinter implements UserAdministratorPrinter {
    ...
    public Object printUsersList(Map<String,Object> params) {
        ...
        String order="1";
        OrderMode ordTipo=OrderMode.ASC;
        ...
        sb.append("<tr>\n");
        //sb.append("<th>\n");
        //sb.append("<a href=\"javaScript:NADataChangeOrder('").append(AuthenticationWay.ID_FIELD).append("', '").append(AuthenticationWay.ID_FIELD.equals(order)&&ordTipo==OrderMode.ASC? "DESC": "ASC").append("')\" />Id</a>\n");
        ...
        sb.append("<a href=\"javaScript:NADefaultDataChangeOrder('").append(AuthenticationWay.LOGIN_FIELD).append("', '").append(AuthenticationWay.LOGIN_FIELD.equals(order)&&ordTipo==OrderMode.ASC? "DESC": "ASC").append("')\" />Usuario</a>\n");
        sb.append("</th>\n");
        sb.append("<th>\n");
        sb.append(AuthenticationWay.NAME_FIELD.toString().equalsIgnoreCase(order)? "<div class=\"NA_data_order"+ordTipo+"\"></div>": "");
        sb.append("<a href=\"javaScript:NADefaultDataChangeOrder('").append(AuthenticationWay.NAME_FIELD).append("', '").append(AuthenticationWay.NAME_FIELD.equals(order)&&ordTipo==OrderMode.ASC? "DESC": "ASC").append("')\" />Nombre</a>\n");
        sb.append("</th>\n");
        sb.append("<th>\n");
        sb.append(AuthenticationWay.MAIL_FIELD.toString().equalsIgnoreCase(order)? "<div class=\"NA_data_order"+ordTipo+"\"></div>": "");
        sb.append("<a href=\"javaScript:NADefaultDataChangeOrder('").append(AuthenticationWay.MAIL_FIELD).append("', '").append(AuthenticationWay.MAIL_FIELD.equals(order)&&ordTipo==OrderMode.ASC? "DESC": "ASC").append("')\" />Mail</a>\n");
        sb.append("</th>\n");
        ...
        for(int i=0; lTmp!=null&&i<lTmp.size(); i++){
            ...
            if( uTmp2!=null ){
                sb.append("<a href=\"#\" id=\"NA:StatusUserButton:").append(lTmp.get(i).getId())
                        .append("\" class=\"NA_UserList_blockedUserButton\" title=\"El usuario esta bloqueado\" onclick=\"NAAdminUsersRemoveBlock('")
                        .append(lTmp.get(i).getId()).append("', '").append(uCon.getSession().getId()).append("')\">&nbsp;</a>\n");                
            }
            //reviso si es monousuario
            if( !ctrl.isMultiUser() && uTmp2==null ){
                sb.append("<a href=\"#\" id=\"NA:StatusUserButton:").append(lTmp.get(i).getId()).append("\" class=\"NA_UserList_")
                        .append(uTmp!=null? "on": "off").append("lineButton\" title=\"").append(uTmp!=null? "En": "Fuera de")
                        .append(" Linea ").append(uTmp!=null? "&#13;Origen:"+uTmp.getOrigin()+"&#13;SO:"+uTmp.getSession().getOs()+"&#13;Browser:"+uTmp.getSession().getBrowser(): "")
                        .append("\" onclick=\"").append(uTmp!=null&&uCon!=null && !uCon.getId().equals(lTmp.get(i).getId())? "NAAdminUsersRemoveConnection('"+lTmp.get(i).getId()+"', '"+uCon.getSession().getId()+"')": "")
                        .append("\">&nbsp;</a>\n");
            }

            //valido que no sea el mismo
            if( uCon!=null && !uCon.getId().equals(lTmp.get(i).getId()) ){
                sb.append("<a href=\"#\" id=\"NA:ActiveUserButton:").append(lTmp.get(i).getId()).append("\" class=\"NA_UserList_")
                        .append(lTmp.get(i).isActive()? "inactive": "active").append("Button\" title=\"")
                        .append(lTmp.get(i).isActive()? "Inhabilita": "Habilita").append(" al usuario\" onclick=\"NAAdminUsers")
                        .append(lTmp.get(i).isActive()? "Inactive": "Active").append("User('").append(lTmp.get(i).getId())
                        .append("', '").append(uCon.getSession().getId()).append("')\" >&nbsp;</a>\n");
            }
