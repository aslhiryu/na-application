...
import neoatlantis.applications.accesscontroller.blockers.ServerMemoryBlocker;
...
import neoatlantis.applications.web.AdminUsersServlet;
import neoatlantis.applications.web.utils.AuthenticationResourcesLoader;
import neoatlantis.applications.web.utils.PhotoUserServlet;
...
public class AccessControllerPublisher implements ServletContextListener{
    ...
    private void finalizeEnvironment(ServletContext context){
        ...
        MBeanRegister.unregisterMBean("neoatlantis.app."+(app!=null? DataUtils.cleanSpecialCharacters(app): "undefined")+".users.jmx:type=InfoStatusUsers");
    }
    ...
    private BlockerWay makeBlocker(ServletContext context, final Element root){
        ...
        if( blo==null ){
            blo=new ServerMemoryBlocker(context);
        }
